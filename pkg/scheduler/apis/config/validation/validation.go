...
func ValidateKubeSchedulerConfiguration(cc *config.SchedulerConfiguration) utilerrors.Aggregate {
	...
	if cc.PercentageOfFoodsToScore < 0 || cc.PercentageOfFoodsToScore > 100 {
		errs = append(errs, field.Invalid(field.NewPath("percentageOfFoodsToScore"),
			cc.PercentageOfFoodsToScore, "not in valid range [0-100]"))
	}
	if cc.DguestInitialBackoffSeconds <= 0 {
		errs = append(errs, field.Invalid(field.NewPath("dguestInitialBackoffSeconds"),
			cc.DguestInitialBackoffSeconds, "must be greater than 0"))
	}
	if cc.DguestMaxBackoffSeconds < cc.DguestInitialBackoffSeconds {
		errs = append(errs, field.Invalid(field.NewPath("dguestMaxBackoffSeconds"),
...
func validatePluginConfig(path *field.Path, apiVersion string, profile *config.KubeSchedulerProfile) []error {
	...
	m := map[string]interface{}{
		...
		"InterDguestAffinity":             ValidateInterDguestAffinityArgs,
		"FoodAffinity":                    ValidateFoodAffinityArgs,
		"FoodResourcesBalancedAllocation": ValidateFoodResourcesBalancedAllocationArgs,
		"FoodResourcesFitArgs":            ValidateFoodResourcesFitArgs,
		"DguestTopologySpread":            ValidateDguestTopologySpreadArgs,
