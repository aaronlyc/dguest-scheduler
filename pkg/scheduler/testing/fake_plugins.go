...
const ErrReasonFake = "Foods failed the fake plugin"
...
func (pl *FalseFilterPlugin) Filter(_ context.Context, _ *framework.CycleState, dguest *v1alpha1.Dguest, foodInfo *framework.FoodInfo) *framework.Status {
...
// Filter invoked at the filter extension point.
func (pl *TrueFilterPlugin) Filter(_ context.Context, _ *framework.CycleState, dguest *v1alpha1.Dguest, foodInfo *framework.FoodInfo) *framework.Status {
	return nil
}
...
// been called, and it returns different 'Code' depending on its internal 'failedFoodReturnCodeMap'.
type FakeFilterPlugin struct {
	NumFilterCalled         int32
	FailedFoodReturnCodeMap map[string]framework.Code
}
...
// Filter invoked at the filter extension point.
func (pl *FakeFilterPlugin) Filter(_ context.Context, _ *framework.CycleState, dguest *v1alpha1.Dguest, foodInfo *framework.FoodInfo) *framework.Status {
	atomic.AddInt32(&pl.NumFilterCalled, 1)

	if returnCode, ok := pl.FailedFoodReturnCodeMap[foodInfo.Food().Name]; ok {
		return framework.NewStatus(returnCode, fmt.Sprintf("injecting failure for dguest %v", dguest.Name))
	}
...
func NewFakeFilterPlugin(failedFoodReturnCodeMap map[string]framework.Code) frameworkruntime.PluginFactory {
	return func(_ runtime.Object, _ framework.Handle) (framework.Plugin, error) {
		return &FakeFilterPlugin{
			FailedFoodReturnCodeMap: failedFoodReturnCodeMap,
		}, nil
	}
}

// MatchFilterPlugin is a filter plugin which return Success when the evaluated dguest and food
...
// Filter invoked at the filter extension point.
func (pl *MatchFilterPlugin) Filter(_ context.Context, _ *framework.CycleState, dguest *v1alpha1.Dguest, foodInfo *framework.FoodInfo) *framework.Status {
	food := foodInfo.Food()
	if food == nil {
		return framework.NewStatus(framework.Error, "food not found")
	}
	if dguest.Name == food.Name {
		return nil
	}
	return framework.NewStatus(framework.Unschedulable, ErrReasonFake)
}
...
func (pl *FakePreFilterPlugin) PreFilter(_ context.Context, _ *framework.CycleState, dguest *v1alpha1.Dguest) (*framework.PreFilterResult, *framework.Status) {
...
func (pl *FakeReservePlugin) Reserve(_ context.Context, _ *framework.CycleState, _ *v1alpha1.Dguest, _ string) *framework.Status {
	return pl.Status
}

// Unreserve invoked at the Unreserve extension point.
func (pl *FakeReservePlugin) Unreserve(_ context.Context, _ *framework.CycleState, _ *v1alpha1.Dguest, _ string) {
}
...
func (pl *FakePreBindPlugin) PreBind(_ context.Context, _ *framework.CycleState, _ *v1alpha1.Dguest, _ string) *framework.Status {
	return pl.Status
}
...
func (pl *FakePermitPlugin) Permit(_ context.Context, _ *framework.CycleState, _ *v1alpha1.Dguest, _ string) (*framework.Status, time.Duration) {
