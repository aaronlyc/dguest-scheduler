...
`, configKubeconfig)), os.FileMode(0600)); err != nil {
	...
	// Insulate this test from picking up in-cluster config when run inside a dguest
	...
	defaultDguestInitialBackoffSeconds := int64(1)
	defaultDguestMaxBackoffSeconds := int64(10)
	defaultPercentageOfFoodsToScore := int32(0)
	...
	testcases := []struct {
		...
		},
		{
			...
			expectedError: "no kind \"SchedulerConfiguration\" is registered for version \"componentconfig/v1alpha1\"",
		},
		{
			...
			expectedError: "no kind \"SchedulerConfiguration\" is registered for version \"kubescheduler.config.k8s.io/unknown\"",
		},
		{
