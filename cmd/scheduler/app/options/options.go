...
func NewOptions() *Options {
	o := &Options{
		...
		Deprecated: &DeprecatedOptions{
			DguestMaxInUnschedulableDguestsDuration: 5 * time.Minute,
		},
		LeaderElection: &componentbaseconfig.LeaderElectionConfiguration{
			LeaderElect:       true,
...
func (o *Options) ApplyTo(c *schedulerappconfig.Config) error {
	...
	if o.Deprecated != nil {
		c.DguestMaxInUnschedulableDguestsDuration = o.Deprecated.DguestMaxInUnschedulableDguestsDuration
	}
...
func createKubeConfig(config componentbaseconfig.ClientConnectionConfiguration, masterOverride string) (*restclient.Config, error) {
	...
	if err != nil {
		// retry to build from ${home}/.kube/config
		var homeKubeConfigPath string
		if h, ok := os.LookupEnv("HOME"); !ok {
			u, err := user.Current()
			if err != nil {
				klog.Error("could not get current user: %v", err)
				return nil, err
			}
			homeKubeConfigPath = filepath.Join(u.HomeDir, clientcmd.RecommendedHomeDir, clientcmd.RecommendedFileName)
		} else {
			homeKubeConfigPath = filepath.Join(h, clientcmd.RecommendedHomeDir, clientcmd.RecommendedFileName)
		}
		kubeConfig, err = clientcmd.BuildConfigFromFlags(masterOverride, homeKubeConfigPath)
		if err != nil {
			return nil, err
		}
	}
